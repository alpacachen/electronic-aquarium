 ...
 */
describe('中英文切换', () => {
  it('中英文往返时界面文案跟着换，标签页标题和文档语言也跟着切，且不会把缸重置掉', async () => {
    // Given 观众打开的是中文界面
    ...
    const fishBefore = aquarium.fish()
    ...
    // 而且那颗按钮标出了当前用的就是英文，标签页和 <html lang> 也都跟着变了
    ...
    /**
     * 而且切语言只换了界面上的字：鱼还是那几条，位置一条都没动，镜头也还在观众
     * 留下它的地方。一旦语言这一层落到了缸里面——Provider 塞进 Canvas，或者拿
     * 语言给 Canvas 当 key——观众点一下语言，鱼就会重新入场、镜头也被拨回默认角度。
     */
    const fishAfter = aquarium.fish()
    expect(fishAfter).toHaveLength(fishBefore.length)
    expect(fishAfter.map(({ position, species }) => ({ position, species }))).toEqual(
      fishBefore.map(({ position, species }) => ({ position, species })),
    )
    expect(aquarium.camera().position.distanceTo(cameraBefore)).toBeLessThan(1e-6)

    // 而且鱼接着往下游，不是停在那儿
    aquarium.letTimePass(1)
    expect(aquarium.fish()[0].position).not.toEqual(fishAfter[0].position)
   ...
   */
  /**
   * 这两条各自开一个缸：openAquarium 挂的树只在 afterEach 里卸载，同一条用例里
   * 开两个缸会把两棵树都留在文档里，按钮名字就成了重复的 locator，谁也点不准。
   */
  ...
  it('没选过的话，头一次进来跟着浏览器偏好，认不出的语言落到英文', async () => {
